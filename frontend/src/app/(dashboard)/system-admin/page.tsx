...
export default function SystemAdminDashboardPage() {
  ...
  }, [activeEnrollments, enrollmentsCount, publishedCount, coursesCount, pendingApprovals, certificatesCount, activeSessions, isAmharic]);
    ...
    >
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={usersCount}
          hint={isAmharic ? `${activeEnrollments} ንቁ ምዝገባዎች` : `${activeEnrollments} active enrollments`}
        />
        <StatCard
          icon={BookOpen}
        ...
        <StatCard
          ...
          hint={isAmharic ? `${certificatesCount} የተሰጡ ሰርተፊኬቶች` : `${certificatesCount} certificates issued`}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {isAmharic ? "በመድረኩ ውስጥ ያሉ መለያዎች ድልድል" : "Allocation of platform accounts across access roles"}
                </p>
              </div>
              <Badge variant="slate">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-soft ring-super-soft flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="blue">
                {isAmharic ? "የቀጥታ ስታቲስቲክስ" : "Real-time stats"}
              </Badge>
            </div>
            <div className="mt-6">
              <BarChart
                items={metricBars}
                emptyText={isAmharic ? "ምንም የስራ መለኪያ አልተመዘገበም።" : "No operational metrics recorded."}
                barColor="#6366f1"
              />
            </div>
          </div>
          <div className="mt-4 border-t border-slate-100 pt-3 text-xs text-slate-500 flex items-center justify-between">
            <span>{isAmharic ? `ጠቅላላ ምዝገባዎች: ${enrollmentsCount}` : `Total enrollments: ${enrollmentsCount}`}</span>
              ...
              <ul className="divide-y divide-slate-100/80">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs text-slate-800">
                        <span className="font-semibold text-slate-900">
                          {log.user ? `${log.user.firstName} ${log.user.lastName}` : (isAmharic ? "የስርዓት አገልግሎት" : "System Service")}
                        </span>{" "}
                ...
                {recentLogs.length === 0 ? (
                  <li className="px-5 py-8 text-center text-xs text-slate-400">
                    {isAmharic ? "ምንም የቅርብ ጊዜ የኦዲት እንቅስቃሴ አልተመዘገበም።" : "No recent audit activity recorded."}
                  </li>
                ) : null}
              </ul>
            </div>
