...
type PlatformChoice =
  'LIVEKIT' | 'JITSI' | 'GOOGLE_MEET' | 'ZOOM' | 'BIGBLUEBUTTON' | 'MS_TEAMS' | 'CUSTOM';
        ...
        >
            ...
            <SearchableCourseSelect
              ...
              onChange={(selectedId) => {
                ...
                if (platformType === 'JITSI') {
                  const code = (
                    courses.find((c) => c.id === selectedId)?.code || 'TRAINING'
                  ).replace(/[^a-zA-Z0-9]/g, '');
                  setExternalUrl(
                    `https://meet.jit.si/MoR-LMS-${code}-${Math.random().toString(36).substring(2, 8)}`,
                  );
                }
              }}
            />
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-4 space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              {trainersLoading ? (
                <span className="text-[11px] font-medium text-indigo-600 animate-pulse">
                  Loading trainers…
                </span>
            ...
            <p className="text-[11px] text-slate-600 leading-relaxed">
              The assigned trainer will be designated as the session speaker, appear with verified
              host credentials in the video classroom, and receive direct scheduling reminders.
            </p>
          </div>
          ...
          {platformType === 'LIVEKIT' ? (
            <div className="rounded-xl border border-indigo-200 bg-indigo-50/70 p-3.5 space-y-1 text-xs">
              ...
              <p className="text-indigo-700">
                No external links or apps required. Participants join directly in the ELTMS
                workspace with native WebRTC audio, video, screen sharing, and automated attendance
                tracking.
              </p>
            </div>
          ) : (
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-2.5">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Attendance calculation and Minimum Active Stay Threshold for &quot;Present&quot;
              status are governed centrally by institutional policy configured in System Admin
              Policies.
            </p>
            <label className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer pt-1">
              ...
              />
              <span>
                Permit participants (all actors) to view attendees and attendance modal for this
                session
              </span>
            </label>
          </div>
